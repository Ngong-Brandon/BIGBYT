// src/pages/Home.jsx
// ─── Authenticated home screen shown after login ──────────────────────────────
...
const PROMOS = [
  { label: "LIMITED TIME",   title: "30% OFF",        sub: "First order from Sakura Express", cta: "Claim Deal",    bg: "#1A0800", border: "#FF450033", ctaBg: C.accent,   ctaColor: "#fff",    ctaText: "#FF4500", icon: "🌸" },
  { label: "FREE DELIVERY",  title: "Orders over $20", sub: "All restaurants this weekend",   cta: "Order Now",     bg: "#0A1200", border: "#00C48C33", ctaBg: "#00C48C",   ctaColor: "#fff",    ctaText: "#00C48C", icon: "🛵" },
  { label: "NEW RESTAURANT", title: "Nonna's Kitchen", sub: "Authentic Italian · Now open",   cta: "Explore Menu",  bg: "#0A0A1A", border: "#F5A62333", ctaBg: "#F5A623",   ctaColor: "#412402", ctaText: "#F5A623", icon: "🍝" },
];
...
function RestaurantMini({ r, onEnter }) {
  return (
    <div onClick={onEnter} className="card-hover"
      style={{ background: "#181818", border: `1px solid ${C.border}`, borderRadius: 16, padding: 16, cursor: "pointer", flex: "1 1 calc(50% - 6px)", minWidth: 140 }}>
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function Home({ go, setActiveRestaurant, activeOrder }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Show first 4 open restaurants as "nearby"
      setNearbyRestaurants((restaurants || []).filter(r => r.is_open).slice(0, 4));
  ...
  return (
    <div style={{ fontFamily: " sans-serif", color: C.text, background: C.bg, paddingBottom: 100 }}>
      <style>{`
        .card-hover:hover { border-color: #FF450055 !important; transform: translateY(-2px); transition: all 0.18s; }
        .cat-pill { transition: all 0.15s; }
        .cat-pill:hover { border-color: #FF450055 !important; color: #F0EBE1 !important; }
      ...
      <div style={{ padding: "28px 20px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 4, fontFamily: "'DM Mono', monospace" }}>
            {getGreeting()} 🔥
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 5 }}>
            ...
            <span style={{ fontSize: 13, color: C.muted }}>
              Lekki Phase 1 · <span style={{ color: C.accent, fontWeight: 700 }}>Lagos</span>
            </span>
          </div>
        </div>
        ...
        <div style={{ position: "relative", cursor: "pointer" }} onClick={() => {}}>
          ...
          <div style={{ position: "absolute", top: -4, right: -4, width: 16, height: 16, background: C.error, borderRadius: "50%", border: `2px solid ${C.bg}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "#fff" }}>3</div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 20px 24px" }}>
        <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 4 }}>
          {PROMOS.map((p, i) => (
            <div key={i} style={{ background: p.bg, border: `1px solid ${p.border}`, borderRadius: 18, padding: "18px 20px", minWidth: 240, position: "relative", overflow: "hidden", flexShrink: 0, cursor: "pointer" }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: p.ctaText, letterSpacing: 2, marginBottom: 8 }}>{p.label}</div>
              ...
              <div style={{ background: p.ctaBg, display: "inline-block", padding: "7px 16px", borderRadius: 8, fontSize: 12, fontWeight: 800, color: p.ctaColor, cursor: "pointer" }}>{p.cta}</div>
              <div style={{ position: "absolute", right: 14, top: 14, fontSize: 42, opacity: 0.15 }}>{p.icon}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "0 20px 24px", display: "flex", gap: 10 }}>
        {[
          { val: recentOrders.length || 0, label: "Orders",    color: C.accent },
          { val: "$0",                     label: "Saved",      color: C.success },
          { val: "4.8",                    label: "Avg Rating", color: "#F5A623" },
          { val: "3",                      label: "Faves",      color: C.text },
        ].map(({ val, label, color }) => (
          <div key={label} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 8px", flex: 1, textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
          {CATEGORIES.map(cat => (
            <div key={cat.label} className="cat-pill"
              ...
              style={{ background: activeCat === cat.label ? `${C.accent}18` : "#161616", border: `1px solid ${activeCat === cat.label ? C.accent : C.border}`, borderRadius: 100, padding: "8px 16px", fontSize: 12, fontWeight: 700, color: activeCat === cat.label ? C.accent : C.muted, cursor: "pointer", whiteSpace: "nowrap" }}>
              {cat.emoji} {cat.label}
            </div>
          ))}
        </div>
      ...
      <div style={{ padding: "0 20px 24px" }}>
        ...
        {loading ? (
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} style={{ flex: "1 1 calc(50% - 6px)", height: 140, background: C.surface, borderRadius: 16, animation: "pulse 1.5s infinite" }} />
            ))}
          </div>
        ) : nearbyRestaurants.length > 0 ? (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
            {nearbyRestaurants.map(r => (
              <RestaurantMini key={r.id} r={r} onEnter={() => enterRestaurant(r)} />
            ))}
          </div>
        ) : (
          <div style={{ textAlign: "center", padding: "40px 0", color: C.muted, fontSize: 14 }}>
            No restaurants near you yet
          </div>
        )}
      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {recentOrders.map(order => (
              <div key={order.id} className="card-hover"
                style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer" }}>
                ...
                <button onClick={() => go("restaurants")}
                  style={{ background: C.accent, color: "#fff", border: "none", borderRadius: 9, padding: "8px 14px", fontSize: 12, fontWeight: 800, cursor: "pointer", fontFamily: "'Syne', sans-serif" }}>
                  Reorder
                </button>
              </div>
            ))}
      ...
      <div style={{ padding: "0 20px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 4 }}>
          {POPULAR_ITEMS.map((item, i) => (
            <div key={i} onClick={() => go("restaurants")}
              style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 14, minWidth: 148, cursor: "pointer", flexShrink: 0 }}
